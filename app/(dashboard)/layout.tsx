...
import { Calendar, Scissors, Users, LogOut, CalendarClock, Menu, X, Sun, Moon, UserCircle } from 'lucide-react';
...
function SidebarContent({ user, navItems, pathname, theme, onToggleTheme, onLogout, onNavClick }: SidebarContentProps) {
  return (
    <>
      <div className="p-5 border-b border-slate-700">
        <p className="text-sm font-bold text-white mb-3">✂️ Barber App</p>
        <Link
          href="/perfil"
          onClick={onNavClick}
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-white text-sm font-medium truncate group-hover:underline">{user.name}</p>
            <span className="text-xs bg-slate-700 rounded-full px-2 py-0.5 text-slate-300">
              {{ admin: 'Administrador', professional: 'Profissional', client: 'Cliente' }[user.role] ?? user.role}
            </span>
          </div>
        </Link>
      </div>
    ...
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden flex items-center gap-3 px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shrink-0">
          ...
          <span className="text-sm font-bold text-gray-900 dark:text-white">✂️ Barber App</span>
          <button
            onClick={toggle}
